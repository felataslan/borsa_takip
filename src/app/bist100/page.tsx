  ...
  useEffect(() => {
    async function fetchStocks() {
      try {
        ...
        const bist100Stocks = data.filter((stock: Stock) => BIST_100.includes(stock.symbol));
        setStocks(bist100Stocks);
      } catch (err) {
    ...
    <Container maxWidth="xl" sx={{ py: 4, minHeight: '100vh', position: 'relative', overflowX: 'clip' }}>
      <Box sx={{ mb: 4, position: 'relative', zIndex: 10 }}>
        <Typography variant="h4" component="h1" sx={{ fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: 1, color: 'text.primary' }}>
          <Map color="#9333ea" size={32} />
          BİST 100 Piyasası
        </Typography>
        <Typography variant="body1" color="text.secondary" sx={{ mt: 1, maxWidth: 'md' }}>
          Borsa İstanbul&apos;un piyasa ve pazar göstergesi sayılan temel 100 hisse senedi listesi.
        </Typography>
      </Box>
      ...
      <Box sx={{ position: 'fixed', top: 80, right: 0, zIndex: -10, height: 256, width: 256, borderRadius: '50%', bgcolor: 'rgba(147, 51, 234, 0.05)', filter: 'blur(120px)', pointerEvents: 'none' }} />
      <Box sx={{ position: 'fixed', bottom: 0, left: 0, zIndex: -10, height: 384, width: 384, borderRadius: '50%', bgcolor: 'rgba(34, 197, 94, 0.05)', filter: 'blur(150px)', pointerEvents: 'none' }} />
    </Container>
  );
}
